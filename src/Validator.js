...
define(function() {
    ...
    function formatStr(source, var_args) {
      source = String(source);

      var opts = arguments[1];
      if ('undefined' != typeof opts) {
        if (lib.isNumber(opts) || lib.isString(opts)) {
          return source.replace('{0}', opts);
        } else {
...
var Validator = (function() {
    ...
    function parse(text, type) {
        if (type === 'int') {
            ...
            return lib.date.isDate(text) ? text : lib.date.parse(text);
        } else {
            return text;
        }
    }
    ...
    var errorClass = 'validate-error',
        ...
        errorMsg = {
            ...
            ERROR_COLOR: '格式不正确，请填写颜色代码',
            ERROR_BACKEND: '{0}',
            ERROR_NOTDEFAULT: '请选择'
        },
        ...
        ruleMap = {
            'required': {
                validate: function(value) {
                    return lib.isEmptyObject(value) ? 'ERROR_EMPTY' : 'SUCCESS';
                }
            },
            ...
            'maxCNCharLength': {
                validate: function(text, maxCNCharLength) {
                    if (lib.getByteLength(text) > maxCNCharLength * 2) {
            ...
            'maxTextCNCharLength': {
                validate: function(html, value) {
                    ...
                    if (lib.getByteLength(text) > value * 2) {
                ...
                validate: function(value) {
                    ...
                    if(indexs.length > 0){
                        ...
                        for(var i = 0; i < indexs.length; i++){
                            msgArr.push(formatStr('第{0}行第{1}列', indexs[i]['rowIndex'], indexs[i]['colIndex']));
                        }
            ...
            'color': {
                validate: function(text) {
                    ...
                    if (!/^[a-zA-Z]+$/.test(text)
                        && !/^#([0-9A-F]{3}){1,2}$/i.test(text)
                        && !/^rgb\s*(\s*[012]?[0-9]{1,2}\s*(,\s*[012]?[0-9]{1,2}\s*){2})$/i.test(text)
                        && !/^rgba\s*(\s*[012]?[0-9]{1,2}\s*(,\s*[012]?[0-9]{1,2}\s*){2},(1|0\.\d+))$/i.test(text)
                    ) {
                        return 'ERROR_COLOR';
                    }
            ...
            // 下拉框不为默认值
            'notdefault': {
                validate: function(value, control) {
                    return value === control.getDefaultValue() ? 'ERROR_NOTDEFAULT' : 'SUCCESS';
                }
            }
        };
    ...
    function noticeInTail(noticeText, input, control) {
        ...
        if (input.tagName == 'INPUT') {
            ...
            title = input.getElementsByTagName('input');
            title = title.length ? title[0].title : '';
        }
    ...
    function showNoticeDom(input) {
        var el = getEl(input);
        var father = _type == 'after' ? input : input.parentNode;
        ...
        lib.addClass(father, errorClass);
    }
    ...
    function cancelNoticeInTile(input) {
        ...
        noError && lib.removeClass(father, errorClass);
    }
    /**
     * 在父元素的末尾取消提示信息
     *
    ...
    function cancelNoticeInTileUploader(input, control) {
        ...
        lib.removeClass(father, errorClass);
        if (father2) {
            lib.removeClass(father2, errorClass);
        }
    }
    ...
    function getTextEl(input) {
        return lib.g(input.id + textSuffix);
    }
    ...
    function applyRule(control, ruleName) {
        ...
        var ruleSeg = lib.isArray(ruleName) ? ruleName : ruleName.split(','),
        ...
        if (segLen > 0) {
            for (i = 1; i < segLen; i++) {
                if (ruleSeg[i] == 'this') {
                    ...
                    try {
                        ctrl = lib.isString(ruleSeg[i]) && ui.util.get(ruleSeg[i], control.getPage());
                    } catch(e) {
                        ctrl = null;
                    }
        ...
        if (lib.isNumber(error) && error !== 0) { //TODO:这种形式是要被历史遗弃的
            errorText = rule.noticeText[error];
        } else if (lib.isString(error) && error !== '') {
            errorText = errorMsg[error];
        } else if (lib.isArray(error)) {
            error[0] = errorMsg[error[0]];
            errorText = formatStr.apply(null, error);
        }
...
Validator.showError = function(ele, errorMsg, opt_width) {
    ...
    lib.addClass(parent, 'validate-error');
    if (parent.lastChild.className !== 'validate') {
        var errorNode = document.createElement('div');
        lib.addClass(errorNode, 'validate');
 ...
 * @param {HTMLElement} ele dom元素.
 */
Validator.hideError = function(ele, opt_type) {
    var parent = opt_type && opt_type == 'after' ? ele : ele.parentNode;
    if (parent && parent.lastChild.className === 'validate') {
        parent.removeChild(parent.lastChild);
    }
    lib.removeClass(ele.parentNode, 'validate-error');
};
